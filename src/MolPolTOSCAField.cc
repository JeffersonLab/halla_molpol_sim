    ...
    for(nY = 0; nY < NY; nY++){
      for(nZ = 0; nZ < NZ; nZ++){
        ...
        if (std::istringstream(inputline) >> raw_xpos >> raw_ypos >> raw_zpos >> raw_B >> raw_Bx >> raw_By >> raw_Bz) {
          nlines++;
        } else if (std::istringstream(inputline) >> raw_xpos >> raw_ypos >> raw_zpos >> raw_Bx >> raw_By >> raw_Bz) {
	      nlines++;
	    } /*else {
          G4cerr << "Error in " << __PRETTY_FUNCTION__ << " ... the file " << fFilename << " contains invalid data." << G4endl;
          G4cout << "  raw_xpos: " << raw_xpos << G4endl;
          G4cout << "  raw_ypos: " << raw_ypos << G4endl;
          G4cout << "  raw_zpos: " << raw_zpos << G4endl;
          G4cout << "     raw_B: " << raw_B    << G4endl;
          G4cout << "    raw_Bx: " << raw_Bx   << G4endl;
          G4cout << "    raw_By: " << raw_By   << G4endl;
          G4cout << "    raw_Bz: " << raw_Bz   << G4endl;
